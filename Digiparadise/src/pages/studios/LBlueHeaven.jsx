    ...
    <div className="bg-blue-50 text-black">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-24">
        <motion.p 
          ...
          className="text-center text-lg md:text-xl text-blue-900 max-w-4xl mx-auto mb-12 sm:mb-24"
        >
        ...
        <div className={`grid ${allImages.length === 1 ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'} gap-12 place-items-center overflow-hidden`}>
          {allImages.map((img, i) => (
            <motion.div
              ...
              className="cursor-pointer p-2 bg-white w-64 h-64 sm:w-80 sm:h-80 md:w-96 md:h-96 rounded-2xl shadow-lg shadow-blue-500/20 border-4 border-white hover:border-blue-300 transition-all duration-300 transform hover:scale-105"
            >
              <img 
                src={img} 
        ...
        <div className="mt-16 sm:mt-32 text-center">
             <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-blue-900 mb-8">Studio Tour</h2>
            <motion.div
                initial={{ opacity: 0, y: 50 }}
            ...
            >
                <video 
                    ...
                    controls 
                    className="w-full rounded-lg shadow-2xl border-2 border-blue-200"
                >
      ...
      <nav className="bg-blue-100 py-6 md:py-8 border-t border-blue-200">
        <div className="max-w-7xl mx-auto px-4 text-center">
            <h3 className="text-xl sm:text-2xl font-bold text-blue-900 mb-6">Explore Other Studios</h3>
            <div className="flex flex-wrap justify-center gap-x-6 gap-y-4">
                {otherStudios.map(studio => (
                    <a key={studio.link} href={studio.link} className="text-blue-800 hover:text-black text-lg transition-colors duration-300">
                        {studio.name}
                    </a>
                ))}
            </div>
      ...
      <AnimatePresence>
        {enlargedImage && (
          ...
          >
            ...
            />
             <button onClick={() => setEnlargedImage(null)} className="absolute top-4 right-4 text-white hover:text-blue-300 z-[51]">
                <X size={32} />
             </button>
          </motion.div>
        )}
