    ...
    <div className="bg-gray-100 text-black">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-24">
        <motion.p 
          ...
          className="text-center text-lg md:text-xl text-gray-700 max-w-4xl mx-auto mb-12 sm:mb-24"
        >
        ...
        <div className={`grid ${allImages.length === 1 ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'} gap-12 place-items-center overflow-hidden`}>
          {allImages.map((img, i) => (
            <motion.div
              ...
              className="cursor-pointer p-2 bg-white w-64 h-64 sm:w-80 sm:h-80 md:w-96 md:h-96 rounded-2xl shadow-lg shadow-gray-500/20 border-4 border-white hover:border-gray-300 transition-all duration-300 transform hover:scale-105"
            >
              <img 
                src={img} 
        ...
        <div className="max-w-7xl mx-auto px-4 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-x-6 gap-y-4">
                {otherStudios.map(studio => (
                    <a key={studio.link} href={studio.link} className="text-gray-600 hover:text-black text-lg transition-colors duration-300">
                        {studio.name}
                    </a>
                ))}
            </div>
      ...
      <AnimatePresence>
        {enlargedImage && (
          ...
          >
            ...
            />
             <button onClick={() => setEnlargedImage(null)} className="absolute top-4 right-4 text-white hover:text-gray-300 z-[51]">
                <X size={32} />
             </button>
          </motion.div>
        )}
